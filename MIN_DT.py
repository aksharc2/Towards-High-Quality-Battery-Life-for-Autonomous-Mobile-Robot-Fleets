...
def TCM_Optimization():
    ...
    if File_Execution == True:
        Setting_df=pd.read_csv(File_name)
        Setting_df.loc[Exp_no,"MIN_DT_max_diff"] = max_diff
        Setting_df.loc[Exp_no,"MIN_DT_Coeff_Var"] = Coeff_Var
        Setting_df.loc[Exp_no,"MIN_DT_Total_Downtime"] = np.sum(Task_Downtime)
        Setting_df.loc[Exp_no,"MIN_DT_total_obj_cost"] = (' %g' % obj.getValue())
        Setting_df.loc[Exp_no,"MIN_DT_obj1"] = ('%g' % obj1.getValue())
        Setting_df.loc[Exp_no,"MIN_DT_obj2"] = ('%g' % obj2.getValue())
        Setting_df.loc[Exp_no,"MIN_DT_Wt_obj2"] = ('%g' % W_obj2.getValue())
        Setting_df.loc[Exp_no,"MIN_DT_run_time"] = Optimizer_Run_Time
        Setting_df.loc[Exp_no,"MIN_DT_energy_effectiveness"] = Opt_Effectiveness
        Setting_df.loc[Exp_no,"MIN_DT"] = 1
        Setting_df.loc[Exp_no,"MIN_DT_BD_C_Edod"] = BD_C_Edod
        Setting_df.loc[Exp_no,"MIN_DT_BD_C_Emax"] = BD_C_Emax
