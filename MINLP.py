...
Execution_File_name = 'Experiment_Results'+'.csv'
# no_e = 9888
# q = 0
# SOC_File_Name = 'Exp'+str(no_e)+'__OPT_SOC_q__'+str(q)+'.csv'
# TA_N0_File_Name = 'Exp_'+str(no_e)+'__OPT_Task_Allocation_Nav-0_q__'+str(q)+'.csv'
# TA_N1_File_Name = 'Exp_'+str(no_e)+'__OPT_Task_Allocation_Nav-1_q__'+str(q)+'.csv'
...
def TCM_Optimizer_Initial_Conditions():    # Parameters created
    ...
    bis_option=0 # 0 obj2 penalize SOC dist from thresholds; 1 obj2 penalizes energy waste; 1 with q=0 only downtown but SOC between thresholds (baseline)
    ...
    Exp_no = Setting_df.iloc[-1,0] + 1
    Setting_df = Setting_df.append({'Experiment_no': Exp_no}, ignore_index=True)
    ...
    Setting_df.loc[Exp_no,"bis_option"] = bis_option
    ...
    Setting_df.loc[Exp_no,"Max_distance"] = str(Max_distance)
    Setting_df.loc[Exp_no,"Dist_change_max"] = Dist_change_max
    Setting_df.loc[Exp_no,"Y"] = str(Y)
    ...
    Opt_Parameters = {'Period_duration': Period_duration, 'Working_Period': Working_Period, 'T': T, 'R': R, 'C': C, 'S': S, 'W': W, 'W_N': W_N, 
                  ...
                  'Sensing_Power':Sensing_Power, 'Robot_Speed':Robot_Speed, 'Max_distance':Max_distance, 'E_changeMax':E_changeMax , 'Exp_no':Exp_no}
...
def TCM_Optimization(Q):
    ...
    for k in Times:
        for i in Robots:
            for c in Charging_stations:
                if bis_option==1:
                    obj2=obj2+E_changeMax*(a[k,i,c]+b[k,i,c])
                else:
...
# m.addGenConstrMin(e[i,p,t], [e_audit[i,p,t], Emax], name="Energy_Balance[%s,%s,%s]" % (i,p, t))  
    if bis_option==1:
        for k in Times:
            for i in Robots:
    ...
    print('Opt_Effectiveness:', Opt_Effectiveness)
    print('Task_Downtime:', np.sum(Task_Downtime))
    ...
    # for i in Robots:
    #     for k in Times:
    #         TA_N_1[k,i] = sum(m.getVarByName('x_(%s,%s,%s,%s)'%(k,i,0,j)).x for j in Non_Navigation_Tasks)
    #         TA_N_2[k,i] = sum(m.getVarByName('x_(%s,%s,%s,%s)'%(k,i,1,j)).x  for j in Non_Navigation_Tasks)
        
    # np.savetxt("Opt_q_1_SOC.csv", state_of_charge, delimiter=",")
    # np.savetxt("Opt_q_1_TA_N_1.csv", TA_N_1, delimiter=",")
    # np.savetxt("Opt_q_1_TA_N_2.csv", TA_N_2, delimiter=",")
    # Allocated_task_df = pd.DataFrame(allocated_tasks)
    # SOC_df = pd.DataFrame(state_of_charge)
    # SOC_df.to_csv(SOC_File_Name)
    
    # TA_N0_df = pd.DataFrame(TA_N_1)
    # TA_N0_df.to_csv(TA_N0_File_Name)
    
    # TA_N1_df = pd.DataFrame(TA_N_2)
    # TA_N1_df.to_csv(TA_N1_File_Name)
    ...
    Setting_df.loc[Exp_no,"Opt_execution"] = 1
    # Setting_df.loc[Exp_no,"SOC_File_Name"] = SOC_File_Name
    Setting_df.loc[Exp_no,"Opt_BD_C_Edod"] = BD_C_Edod
    Setting_df.loc[Exp_no,"Opt_BD_C_Emax"] = BD_C_Emax

    Setting_df.to_csv(Execution_File_name, index = False)

    print('BD_C: ', BD_C_Edod + BD_C_Emax)
...
if __name__ == '__main__':
    if File_Execution == False:
        ...
        Setting_df=pd.read_csv(Execution_File_name)
        loop = int(Setting_df.iloc[-1,0])
        for Exp_no in range(0,loop+1):
            print('\n----------------------- Exp_no:' , Exp_no , '-----------------------\n')
            if Setting_df.loc[Exp_no,"Opt_execution"] == 1:
                print("Optimizer has executed ",Exp_no ," parameters")
