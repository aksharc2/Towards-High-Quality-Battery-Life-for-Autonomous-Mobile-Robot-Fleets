...
def TCM_Parameters(Exp_no, robots, navigations, distance, min_soc,  Initial_charge, max_soc):  
    ...
    if Error_introduced == 1:
        for i in Robots:
            for k in Ex_Times:
                modelling_error[i][k] =  round(random.uniform(error_lower_limit/100, error_upper_limit/100), 2)
    ...
    Setting_df= {'Experiment_no':Exp_no, 'q' : q, 'Period_duration': Period_duration, 'Working_Period': Working_Period, 'No_of_robots':R, 'No_of_chargers':C, 'No_of_sensors':S, 'No_of_non_nav_tasks':W, 
                                      ...
                                      'E_Balance_Zero':E_Balance_Zero, 'E_changeMax':E_changeMax, 'Priority':Priority, 'Sensing_Power':Sensing_Power, 'modelling_error':modelling_error}
...
if __name__ == '__main__' :
    ...
    setup = pd.DataFrame()
# Provide initial setup vales here
